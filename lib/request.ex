defmodule Kogia.Request do
  ...
  def put(client, path, body \\ "{}", headers \\ @put_header, options),
    do: request(client, :put, path, body, headers, options)
  ...
  """
  @spec stream(Client.t(), Atom.t(), String.t(), Map.t(), Keyword.t()) ::
          {:ok, GenEvent.Stream.t()} | {:error, HTTPoison.Error.t()}
  def stream(client, method, path, body \\ "", headers \\ @default_headers, options \\ []) do
    ...
    case HTTPoison.request(
           ...
           body,
           headers,
           options ++ [stream_to: spawn(fn -> stream_loop(listener) end)]
         ) do
  ...
  defp stream_loop_json(status, acc) do
    receive do
      %HTTPoison.AsyncStatus{code: new_status} ->
        stream_loop_json(new_status, acc)

      %HTTPoison.AsyncHeaders{} ->
        stream_loop_json(status, acc)

      %HTTPoison.AsyncChunk{chunk: chunk} ->
        stream_loop_json(status, [Poison.decode!(chunk) | acc])
    ...
    after
      30_000 -> {:error, "Request timed out"}
    end
  end
  ...
  defp body_parser(%HTTPoison.Response{body: body, status_code: status_code})
